...
import { findOrderByIdWithItemsAndUser, markOrderAsPaidAndCreateTransaction } from "@/lib/repositories/order-repository";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. Fetch Order
    const orderRecord = await findOrderByIdWithItemsAndUser(orderId);
    ...
    await markOrderAsPaidAndCreateTransaction(orderId, orderRecord.total, razorpay_payment_id);
